...
const NAV_LINKS = [
  { href: "/", label: "Home" },
  { href: "/rackets", label: "Browse Rackets" },
  { href: "/sell", label: "Sell Your Racket" },
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-lg border-b border-surface-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <Link href="/" className="flex items-center gap-2.5 group">
            ...
            <span className="font-display text-xl text-surface-900">
              Padel<span className="text-brand-600">Market</span>
            </span>
          </Link>
        ...
        {/* Mobile nav */}
        {mobileOpen && (
          <nav className="md:hidden pb-4 border-t border-surface-100 pt-3 space-y-1">
            {NAV_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
